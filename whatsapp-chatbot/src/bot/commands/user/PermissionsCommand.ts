...
export class PermissionsCommand extends Command {
  ...
  private getUserCommands(userType: UserType): {
    ...
    const userLevels = this.getValueByPath(
      null,
      "messages.commands.permissions.user_levels"
    ) || {
    ...
    const commandsByLevelConfig =
      this.getValueByPath(
        null,
        "messages.commands.permissions.commands_by_level"
      ) || {};
  ...
  private getUserRestrictions(userType: UserType): {
    ...
    const restrictionsConfig = this.getValueByPath(
      null,
      "messages.commands.permissions.restrictions"
    );
  ...
  async execute(context: CommandContext): Promise<CommandResult> {
    try {
      ...
      const responseConfig = this.getValueByPath(
        null,
        "messages.commands.permissions.response"
      );
      const userLevels = this.getValueByPath(
        null,
        "messages.commands.permissions.user_levels"
