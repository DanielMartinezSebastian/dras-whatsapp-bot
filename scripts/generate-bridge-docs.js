...
class BridgeDocumentationGenerator {
  ...
  analyzeBridge() {
    ...
    console.log(`📊 Análisis completado: ${this.endpoints.size} endpoints, ${this.structs.size} estructuras`);
  }
  ...
  extractEndpoints(content) {
    const handlerRegex = /http\.HandleFunc\("([^"]+)",\s*func\([^)]+\)\s*{([^}]+(?:{[^}]*}[^}]*)*)/g;
    let match;
...
`;
    ...
    fs.writeFileSync(path.join(this.outputDir, 'flow-diagrams.md'), mermaidDiagram);
  ...
  generateTypeCoverage() {
    ...
    for (const [path, endpoint] of this.endpoints) {
      ...
      if (endpoint.requestType && endpoint.responseType) coverage.endpoints.fullyTyped++;
    }
    ...
    for (const [name, struct] of this.structs) {
      ...
      coverage.structs.fieldsCovered += struct.fields.filter(f => f.description).length;
    }
...
| Endpoints con request types | ${coverage.endpoints.withRequestTypes} | ${Math.round(coverage.endpoints.withRequestTypes / coverage.endpoints.total * 100)}% |
| Endpoints con response types | ${coverage.endpoints.withResponseTypes} | ${Math.round(coverage.endpoints.withResponseTypes / coverage.endpoints.total * 100)}% |
| Endpoints completamente tipados | ${coverage.endpoints.fullyTyped} | ${Math.round(coverage.endpoints.fullyTyped / coverage.endpoints.total * 100)}% |
| **Estructuras totales** | ${coverage.structs.total} | 100% |
| Estructuras documentadas | ${coverage.structs.withDocumentation} | ${Math.round(coverage.structs.withDocumentation / coverage.structs.total * 100)}% |
| Campos documentados | ${coverage.structs.fieldsCovered} / ${coverage.structs.totalFields} | ${Math.round(coverage.structs.fieldsCovered / coverage.structs.totalFields * 100)}% |

## Detalle por Endpoint

| Endpoint | Método | Request Type | Response Type | Estado |
|----------|--------|--------------|---------------|--------|
${Array.from(this.endpoints.entries()).map(([path, endpoint]) => {
  const hasRequest = endpoint.requestType ? '✅' : '❌';
  const hasResponse = endpoint.responseType ? '✅' : '❌';
  const status = endpoint.requestType && endpoint.responseType ? '✅ Completo' : '⚠️ Parcial';
...
|------------|--------|-------------|-----|
${Array.from(this.structs.entries()).map(([name, struct]) => {
...
`;
  ...
  extractStructDescription(content, structName) {
    ...
    const structRegex = new RegExp(`//.*\\n(?://.*\\n)*\\s*type\\s+${structName}\\s+struct`, 'g');
    const match = structRegex.exec(content);
    
    if (match) {
      const comments = match[0].split('\\n')
  ...
  generateEndpointDescription(endpoint) {
    let desc = endpoint.description || \`Endpoint \${endpoint.method} \${endpoint.path}\`;
...
**Request:**
\${endpoint.requestType ? \`Tipo: \\\`\${endpoint.requestType}\\\`\` : 'Sin tipo específico'}
...
\${struct.fields.map(field => 
  \`| \\\`\${field.name}\\\` | \\\`\${field.type}\\\` | \\\`\${field.jsonTag}\\\` | \${field.description || 'Sin descripción'} |\`
).join('\\n')}
